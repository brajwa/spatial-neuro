...
# parameter: kde is the given density function
...
predictKDE <- function(kde, data, mode){
  ...
  if(mode == 4){
    return(predict(kde, x=data$weight))
  }
  if(mode == 5){
    return(predict(kde, x=c(data$anglecomp, data$euclidDist, data$weight)))
  }
}

deterministicEdges <- function(ganglia_ppp, branch.all){
  ...
  # visualize the bivariate distribution of the initial triangulation
  ...
  print(doc, target = paste(output_folder_path, "Tri_Simulated.pptx", sep=""))
...
rejectionSampling <- function(ganglia_ppp, network_extra, g2_degree, orgKDE_angle, orgKDE_length, orgKDE_both,
  ...
  # the edge removal algorithm; for the edges in the triangulation
  rejected1 = 0
  set.seed(Sys.time())
  noChange = 0
  ...
  while (TRUE) {
    ...
    cat("index: ", i, "num of edge: ", length(network_extra[, 1]), "\n")
    
    # check the degree of the nodes of the selected edge; if removing the edge disconnects the network - skip
    ...
    if(fromDeg <= 3 || toDeg <= 3){
      if(dist_to_boundary[network_extra[i, ]$ind1] > 35 && dist_to_boundary[network_extra[i, ]$ind2] > 35){
    ...
    if(uniRandom <= predictedProb){
      ...
      if(is_connected(g2)){
        ...
        #   g2.lin <- linnet(simulated.ppp, edges=as.matrix(network.extra[, 5:6]))
        #   par(mar=c(0,0,0,0), oma=c(0,0,0,0))
        #   plot(simulated.ppp, cex=1, pch=20, main="", bg=1)
        #   plot(g2.lin, add=T)
        # 
        #   Sys.sleep(1)
        # }
