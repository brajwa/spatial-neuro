...
all_face_features = data.frame(matrix(nrow = 36, ncol = 0))

for (f in c(1:length(face_feature_list))) {
    select_feature = f
    cat("Face feature under consideration: ", face_feature_list[select_feature], "\n")
    
    columns = c(paste("mean_", face_feature_list[select_feature], sep = ""),
                paste("sd_", face_feature_list[select_feature], sep=""),
    ...
    for (i in c(1:length(moment_files))) { #2,13,21
        ...
        feature_info_combined = rbind(feature_info_combined, data.frame(mean(feature_info), 
                                                                        sd(feature_info),
                                                                        ens_location, 
                                                                        sample_id) )
    ...
    all_face_features = cbind(all_face_features, feature_info_combined)

}

all_face_features = all_face_features[, !duplicated(colnames(all_face_features), fromLast = TRUE)] 
write.csv(all_face_features, paste(figure_folder, "FaceFeaturesperSample.csv", sep = ""))
###########################################################################################################################
...
all_net_features = data.frame(matrix(nrow = 36, ncol = 0))

for(f in c(1:length(network_feature_list))){
    select_feature = f
    ...
    columns = c(paste("mean_", network_feature_list[select_feature], sep=""),
                paste("sd_", network_feature_list[select_feature], sep=""),
                "ens_location","sample_id") 
    feature_info_combined = data.frame(matrix(nrow = 0, ncol = length(columns))) 
    ...
    for (i in c(1: length(branch_info_files))) { # 2,13,21
       ...
       if(select_feature == 1){
           feature_info_combined = rbind(feature_info_combined, data.frame(mean(igraph::degree(g1)), 
                                                                           sd(igraph::degree(g1)), 
                                                                           ens_location=ens_location, 
                                                                           sample_id=sample_id) )
           ...
           edge_count = E
           feature_info_combined = rbind(feature_info_combined, data.frame(mean(branch.all$angle), 
                                                                           sd(branch.all$angle), 
                                                                           ens_location=ens_location, 
                                                                           sample_id=sample_id) )
           ...
           edge_count = E
           feature_info_combined = rbind(feature_info_combined, data.frame(mean(branch.all$euclid), 
                                                                           sd(branch.all$euclid), 
                                                                           ens_location=rep(ens_location, 1), 
                                                                           sample_id=rep(sample_id, 1)) )
           
       }else if(select_feature == 4){
           face_count = length(face_area_list)
           feature_info_combined = rbind(feature_info_combined, data.frame(mean(face_area_list), 
                                                                           sd(face_area_list), 
                                                                           ens_location=rep(ens_location, 1), 
                                                                           sample_id=rep(sample_id, 1)) )
       }else if(select_feature == 5){
           face_count = length(face_area_list)
           feature_info_combined = rbind(feature_info_combined, data.frame(mean(face_node_count), 
                                                                           sd(face_node_count), 
                                                                           ens_location=rep(ens_location, 1), 
                                                                           sample_id=rep(sample_id, 1)) )
       }
    ...
    colnames(feature_info_combined) = columns
    all_net_features = cbind(all_net_features, feature_info_combined)

}

all_net_features = all_net_features[, !duplicated(colnames(all_net_features), fromLast = TRUE)] 
write.csv(all_net_features, paste(figure_folder, "NetFeaturesperSample.csv", sep = ""))
##########################################################################################################################