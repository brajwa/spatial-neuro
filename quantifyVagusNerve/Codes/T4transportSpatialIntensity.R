...
i = 1
while (i <= length(data_files)) {
  sinkhorn_info = data.frame(matrix(0, nrow = 8, ncol = length(data_files))) # 8 rows, one for each orientation, 
                                                                              # because we are rotating by 45 degree
  ...
  while (j <= length(data_files)) {
    ...
    while(theta < 360){
      ...
      col_info[length(col_info) + 1] = dist_t4$distance
...
openxlsx::saveWorkbook(wb2, file =paste(parent, "Data/Supporting Files/Angle of Rotation/Angle_of_rotation_T4transport_", version, "_", scaling, "_", lambda,".xlsx", sep=""), 
                       overwrite = TRUE)
