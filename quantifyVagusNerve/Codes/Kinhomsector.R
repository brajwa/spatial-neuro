"Linhomsector" <- function(X, ..., correction,
                           verbose=TRUE, begin=0, end=360, units=c("degrees", "radians"), domain = NULL) {
  if(missing(correction)) correction <- NULL
  
  K <- Kinhomsector(X, ..., correction=correction,
                    verbose=verbose, begin=begin, end=end, units=units, domain = domain)
  
  L <- eval.fv(sqrt(pmax.int(K,0)/pi))
  
  # relabel the fv object
  L <- rebadge.fv(L, quote(L[inhom-sect](r)), c("L", "inhom-sect"),
...
"Kinhomsector"<-
            ...
            verbose=TRUE, begin=0, end=360, units=c("degrees", "radians"), domain = NULL)
  {
      ...
      if(missing(lambda) && is.null(reciplambda)) {
        ...
        if(is.im(lambda)) 
          ...
          if(!update) {
            ...
            danger <- FALSE
            if(miss.update) 
              warn.once(key="Kinhomsector.update",
                        "The behaviour of Kinhomsector when lambda is a ppm object",
                        "has changed (in spatstat 1.37-0 and later).",
                        "See help(Kinhomsector)")
          }
    ...
    if(will.do.fast) {
      ## Compute Kinhomsector using fast algorithm(s)
      ...
      ## tweak labels
      K <- rebadge.fv(K, quote(K[inhom-sect](r)), c("K", "inhom-sect"))
      if(danger)
    ...
    K <- ratfv(K, NULL, denom,
               argu="r",
               ylab=quote(K[inhom-sect](r)),
               valu="theo",
               fmla=NULL,
               alim=alim,
               labl=c("r","{%s[%s]^{pois}}(r)"),
               desc=desc,
               fname=c("K", "inhom-sect"),
               ratio=ratio)
    
    # identify all close pairs
    rmax <- max(r)
    what <- if(any(correction == "translate")) "all" else "ijd"
    close <- as.data.frame(closepairs(X, rmax, what=what)) # Maximum distance between pairs of points to be counted as close pairs.
    ...
    if(any(correction == "border" | correction == "bord.modif")) {
      ...
      if(any(correction == "bord.modif")) {
        Kbm <- RS$numerator/eroded.areas(W, r, subset=domain) #new
        if(renormalise)
...
Kwtsum <- function(dIJ, bI, wIJ, b, w, breaks, fatal=TRUE) {
  #
  # "internal" routine to compute border-correction estimates of Kinhomsector
  #
