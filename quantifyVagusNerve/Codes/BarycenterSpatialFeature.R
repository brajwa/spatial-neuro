...
m = openxlsx::read.xlsx(xlsxFile = paste(parent, "Data/Supporting Files/Angle of Rotation/Angle_of_rotation_Barycenter_", version, "_", scaling, "_", lambda,".xlsx", sep=""), 
...
while (i <= length(data_files)) {
  ...
  while (j <= length(data_files)) {
    ...
    distance_matrix[i, j] = skh_bary$Distance
